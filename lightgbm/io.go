...
func lgTreeFromReader(reader *bufio.Reader) (lgTree, error) {
	...
	for len(origNodeIdxStack) > 0 { // 这里为什么要这么复杂呢？直接遍历不可以么，还是说node的顺序很重要
		convIdx := convNodeIdxStack[len(convNodeIdxStack)-1]
		if t.nodes[convIdx].Flags&rightLeaf == 0 { // 这里是表明右节点不是叶子节点
	...
	if loadTransformation && !e.averageOutput {
		...
		if !strings.HasPrefix(objectiveStr, "regression") { // no transformation for regression
			...
			if objectiveStruct.name == "binary" && objectiveStruct.param == "sigmoid" {
				...
				transform = &TransformSoftmax{objectiveStruct.value}
...
func unmarshalTree(raw []byte) (lgTree, error) {
	...
	createCategoricalNode := func(nodeJSON *lgNodeJSON) (lgNode, error) {
		...
		if nBits == 0 {
			...
			bitset := ConstructBitset(thresholdValues)
			if len(bitset) == 1 {
				catIdx = bitset[0]
				catType = catSmall
			} else {
